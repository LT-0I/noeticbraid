...
> Status snapshot updated 2026-05-11 (γ 阶段). `PROJECT_DEFINITION_v3.2.md` is the current spec source of truth.
...
| 字段 | 值 |
|---|---|
| 当前阶段 | v3.2 γ 阶段文档/状态层清债；Phase 1.2 SP integrations through SP-E scheduler landed；双 demo 开发前 D-2 |
| 最后 noeticbraid commit (HEAD) | `bd572cb` (`chore: extend .gitignore + add .gitattributes for Linux migration`) |
| 最近 module integration commits | `33b1d55` / `91fdd77` (SP-E scheduler) ← `c36cb17` (SP-B multimodel) ← `44d04c7` (SP-D obsidian)；另有 `b11f203` (SP-C1 account)、`5faef09` (SP-C2 runtime)、`e2a9a6f` (SP-H NotebookLM) |
| 当前 contract version | Backend API `1.2.0` (`packages/noeticbraid-backend/src/noeticbraid_backend/contracts.py::CONTRACT_VERSION`)；最近 contract tag `phase-1.2-contract-1.3.0` 是 Obsidian sidecar/schema freeze，不改变 backend API version |
| 主要 tag | `phase-1.2-contract-1.2.0` / `phase-1.2-contract-1.3.0` / `phase-1.2-SP-C1-account-quota-1.0.0` / `phase-1.2-SP-C2-runtime-1.0.0` / `phase-1.2-SP-D-obsidian-1.0.0` / `phase-1.2-SP-H-notebooklm-1.0.0` / `phase-1.2-SP-E-scheduler-1.0.0` |
| Backend endpoints | 8 frozen routes (`FROZEN_ROUTE_SPECS`: health / auth / dashboard / workspace / approval / account / ledger runs / ledger aggregate) |
| Schemas (contract 1.2.0) | 17 OpenAPI schema names (`ALL_SCHEMA_NAMES`) |
| pytest core | PASS 424 (last recorded gate; not re-run in γ) |
| pytest backend | PASS 74 (last recorded gate; not re-run in γ) |
| CI 最近一次跑 | run `25381385524` PASS (last recorded gate; not re-run in γ) |
| pending_actor (state.json) | `idle` in `noeticbraid-workflow/state.json`; γ task active as external doc/status cleanup package |
...
| §8 | 多模型路由 + 辩论 | ⚠ 部分入库 | `noeticbraid/packages/noeticbraid-multimodel-alliance` + backend contract 1.2.0 `ModelRoute` schema name | 真实手工对抗辩论闭环、ledger/candidate 写入仍未完成（δ） |
...
| §10 | 神圣红线 | ⚠ 部分 | source_record.py 的 raw/source-only validator + `noeticbraid-obsidian` path/write policy | 整体 vault 写入闭环未启用；SideNote §11.b.S metadata / opt-out / tone 加固仍未完成（δ） |
...
| §14 | Obsidian 管理中心 | ⚠ 部分入库 | `noeticbraid/packages/noeticbraid-obsidian` schemas/templates/path policy/writer | Dashboard / 聊天调度入口 / 文件自动归档 / AI 旁注侧栏 / 报告自动生成未完整串通；§11.b.S 加固留 δ |
...
| §20 | 关键数据对象 | ⚠ partial semantic coverage; field divergence remains | Backend contract 1.2.0 publishes Task / RunRecord / SourceRecord / ApprovalRequest / SideNote / DigestionItem / Workflow / ModelRoute / VaultLayoutMinimum / RunRecordAggregate plus route wrappers；详见 §6 字段对齐表 | RunRecord 与蓝图 §20 聚合字段仍不对齐；SideNote §11.b.S 安全字段仍需 δ 加固 |
...
| §21 (test-suite) | CI / 守门 | ⚠ 部分 | 17 backend schema names + contract route guard + leak scan + last recorded 424 core / 74 backend pytest + CI gate | 仅覆盖 schema/契约/泄漏；不覆盖运行时调度/产物/记忆/成长 |
...
- **Module / SP integrations** — 部分进入主仓，部分裁决归档
  ...
  - obsidian_hub → `44d04c7` (tag `phase-1.2-SP-D-obsidian-1.0.0`) ✅ main package exists; §11.b.S 加固留 δ
  - notebooklm_bridge → `e2a9a6f` / `2a9e91f` (tag `phase-1.2-SP-H-notebooklm-1.0.0`) ✅
  - multimodel_alliance → `c36cb17` ✅ main package exists;真实辩论闭环留 δ
...
`PROJECT_DEFINITION_v3.2.md` 已取代旧蓝图作为 spec source of truth。γ 阶段正在做文档/状态层清债：同步 contract 状态、登记 4 模块归属、归档旧 `noeticbraid-workflow/tools/` 原型。

### 计划

- δ 阶段：补 multimodel 真实手工对抗辩论闭环 + run ledger / candidate 写入
- δ 阶段：按 §11.b.S 加固 SideNote metadata / tone / user_response_channel / opt-out，并做 Obsidian demo fixture
...
- 第二阶段或更后：信息雷达、多源抓取、完整 capability registry、cron/autonomous 调度、NotebookLM 深接入
