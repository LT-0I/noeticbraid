...
`PROJECT_DEFINITION_v3.2.md` 已取代旧蓝图作为 spec source of truth。δ demo day-0 (2026-05-12) 已闭环 5 SDD + 1 hotfix（D1-01 / D1-02 / D2-01 + hotfix-01 / D2-02 / D2-03）+ D1-02-hotfix-02 doc-only polish。Part 2.C 全部闭合：#1/#2/#3/#5 (D1-01/D1-02/D2-02/D2-03, day-0) + #6 candidate→confirmed gate 全局化 (D2-04 `fdf5f81`) + #4 SideNote opt-out 交互 (D2-05 `dbd5da4`) + #7 External Reference Pool 边界裁定 (radar scope = AI 元知识 only，领域 DB → 第二阶段)。δ day-2 D5-01..07 RPC 迁移完结，δ day-3 D6-01..04 vault 入库层完结。当前 (day-3/14) 推进 §4.2 用户成长环内容生成链（D6-05 反向投影 → D6-06 三层质疑 → D6-07 DigestionItem 状态机 → D6-08 报告体系）。14-day demo timer day-14=2026-05-26。

### 计划

- Part 2.C 全部闭合（#1-#7，详见 §当前）；δ 阶段焦点转入 §4.2 用户成长环内容生成链：
  - D6-05 §4.2.2 反向投影引擎（决定"问什么"；当前 SideNote schema 仅数据载体）
  - D6-06 §4.2.3 旁注质疑三层生成器（事实/解释/行动 → SideNote 内容，喂已锁定的 detector→sidenote→vault 链）
  - D6-07 §4.2.4 DigestionItem 追踪状态机（active/resolved/recurring 串通）
  - D6-08 §4.2.5 用户成长报告体系（投影层；scope 至 demo-critical cadence）
