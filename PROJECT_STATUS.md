...
> Status snapshot updated 2026-05-15 (δ demo day-3; D6-04 @ `f32ba7d`). `PROJECT_DEFINITION_v3.2.md` is the current spec source of truth.
...
- `2026-05-15` (δ demo day-3): **D6 系列 SourceRecord/SideNote → Obsidian vault 入库层落地**（SDD-D6-01..04，全程 dual-review）。补上 §10.2 旁注 demo 长期空缺的"vault 写入器"：D5 serializer 产出 SourceRecord dict → D6-01 `ingest_source_record`（单条 policy-checked 入库，commit `25cac32`）→ D6-02 `ingest_serialized_records`（批量幂等 fan-in + env→vault_root 解析收口 + 只读 `IngestSummary` 证据，commit `c79de72`）→ D6-03 `ingest_side_note`（SideNote dict → R-3 旁注子树 `20_ai_observations/side_notes/`，render_side_note 为 §11.b.S 校验器，无 jsonschema 步，commit `13c9b98`）→ D6-04 跨契约回归锁（test-only：`CandidateB1SideNote.to_sidenote(nid).model_dump(mode="json")` → `ingest_side_note` 端到端钉死 3 处脆弱点，commit `f32ba7d`）。obsidian `__all__` 19→23；obsidian pytest 34→85（D6-01 +18 / D6-02 +12 / D6-03 +13 / D6-04 +8）；5-包计量 1151，7-包合计 1201。零新策略（全委托）、零跨包改动、D6-02/03/04 零新依赖、workspace 仍 7 包；backend contract 仍 `1.4.0`（D6 不碰 backend）。红线全保：R-3 路径锁死（sources `30_run_ledger/20_sources/` + side_notes `20_episodic_memory/20_ai_observations/side_notes/`）、ModeEnforcer 拒 user_raw、不改用户原文；R-4 create-only + dry_run 默认。HEAD `f32ba7d`（`25cac32` D6-01 ← `c79de72` D6-02 ← `13c9b98` D6-03 ← `f32ba7d` D6-04）。所有 frozen contract byte-equal；contract_diff / private_leak_scan gate 全绿。**§10.2 vault-写入器缺口已彻底关闭**（D6-03 写入器 + D6-04 跨契约 CI 锁）。详见 §2 "D6 系列 closures (2026-05-15)" + `1workflowR&D/specs/SDD-D6-0{1,2,3,4}-*.md`。
...
| 字段 | 值 |
|---|---|
| 当前阶段 | v3.2 δ demo **day-3** (2026-05-15；14-day timer day-0=2026-05-12 / day-14=2026-05-26)；Phase 1.2 SP integrations 完结；δ day-0 后续 SDD 全部落地：D2-04 (R-6 gate 全局化, Part 2.C #6) / D2-05 (SideNote opt-out 交互, Part 2.C #4) / D2-06 (ChatGPT Web real ping) / D3-01 (real OMC knowledge extraction) / D4-01 (OMC task reuse loop) + 各 hotfix；**D5-01..07 NotebookLM browser→RPC 迁移完结**（bridge 退役）；**D6-01..04 SourceRecord/SideNote→Obsidian vault 入库层落地**（§10.2 旁注 demo vault 写入器缺口已彻底关闭：D6-03 `ingest_side_note` 写入器 + D6-04 跨契约 CI 锁，详 §0.0 / §2）；console UI-SPEC-console-v1 redesign 落地；§10.2 sidenote demo P1 verified；进入 14 天观察期 day-3/14 |
| 最后 noeticbraid commit (HEAD) | `f32ba7d` (`test(obsidian): SDD-D6-04 — lock b1-candidate→SideNote→ingest_side_note cross-contract`) |
| 最近 module integration commits | δ day-3 (2026-05-15) D6 系列：`f32ba7d` (D6-04 跨契约回归锁) ← `13c9b98` (D6-03 ingest_side_note) ← `c79de72` (D6-02 ingest_serialized_records) ← `25cac32` (D6-01 ingest_source_record) ← `b9337a5` (status §1 reconcile δ day-2) ← `db56ccb` (status D5 record)；δ day-2 (2026-05-14) D5 系列：`9dcae5d` (D5-07 删 bridge) ← `7b21faa` (D5-06) ← `6e12535` (D5-05) ← `de71ecd` (D5-04) ← `d02e65d` (D5-03) ← `05bb624`/`7bcf26d` (D5-02+hotfix-02) ← `1f626ea`/`bcbbe78` (D5-01+hotfix-01)；δ day-0..1 另落 D2-04 `fdf5f81`(+hotfix `99e05f2`) / D2-05 `dbd5da4`(+hotfix `1c3e013`) / D2-06 `0ac2e1a`(+hotfix `35f91a7`) / D3-01 `499c335`(+hotfix `48a342d`) / D4-01 `801d562`(+hotfix `0ececb2`) / console `aa52d0e` / CI-leak-allowlist `3ef55c6` |
...
| pytest obsidian | PASS 85 (2026-05-15 δ day-3 全量重跑；D6-01 +18→52 / D6-02 +12→64 / D6-03 +13→77 / D6-04 +8→85；obsidian `__all__` 19→23) |
| pytest notebooklm-rpc | PASS 323 (D5-01..07；2026-05-14 δ day-2 全量重跑) |
| pytest workflow-scheduler / runtime | PASS 29 / 21 (沿用 δ day-2 值，D6 不碰这两包)；5-package 计量 1151 (rpc 323 + core 533 + backend 148 + multimodel 62 + obsidian 85)；7-package 合计 1201 |
...
| pending_actor (state.json) | `idle` in `noeticbraid-workflow/state.json`；δ day-3 D6-01..04 + δ day-2 D5-01..07 + δ day-0..1 全部 SDD 已落地并 push origin main（tag `phase-1.2-SP-H-notebooklm-rpc-2.0.0` @ `9dcae5d` 未新增）；HEAD `f32ba7d` |
...
- **D6 系列 closures (2026-05-15, δ demo day-3)** — SourceRecord/SideNote → Obsidian vault 入库层，全程 dual-review（codex spec-review 多轮 + code-reviewer impl-review）。**§10.2 旁注 demo "vault 写入器未实现" 缺口已彻底关闭**（D6-03 写入器 + D6-04 跨契约 CI 锁）
  - SDD-D6-01 `ingest_source_record(record, *, vault_root, settings=None, body="", project="default") -> WriteResult`：单条 SourceRecord dict → schema 校验 → render → create-only policy-checked 写入；obsidian `__all__` 19→20，+18 tests（obsidian 34→52）；obsidian pyproject +`jsonschema>=4.0,<5.0`（唯一 intentional 非 byte-equal）→ commit `25cac32`（codex spec-review round-2 APPROVE-with-MINOR；impl-review APPROVE）
  - SDD-D6-02 `ingest_serialized_records(records, *, vault_root=None, settings=None, project="default", on_existing="skip") -> IngestSummary`：批量幂等 fan-in（委托 D6-01）+ env→vault_root 解析收口（D6-01 刻意外置部分）+ 只读 frozen `IngestSummary` 证据；`on_existing="skip"`(默认，create-only 命中跳过绝不覆盖)/"raise"；obsidian `__all__` 20→22，+12 tests（obsidian 52→64）；零新依赖（pyproject byte-equal）、零跨包改动、D6-01 函数体 byte-equal → commit `c79de72`（codex spec-review round-1 REJECT-with-CRIT → rev2 round-2 APPROVE；impl-review APPROVE-with-MINOR，2 MINOR 均 spec-faithful 非阻塞）
  - SDD-D6-03 `ingest_side_note(record, *, vault_root, settings=None, body="", project="default") -> WriteResult`：SideNote dict → R-3 旁注子树 `{ns}/20_episodic_memory/20_ai_observations/side_notes/{YYYY}/{MM}/{safe_id}.md` create-only 写入；`render_side_note` 即 §11.b.S 权威校验器（side_note.schema.json 1.3.0 与 §11.b.S 2.0.0 不兼容且不在 render 路径上 → 刻意无 jsonschema 步）；obsidian `__all__` 22→23，+13 tests（obsidian 64→77）；零新依赖（pyproject byte-equal）→ commit `13c9b98`（codex spec-review：初版用 Draft7Validator 必失败，PLAN 阶段探查捕获并改 no-jsonschema 设计，round-1 APPROVE；impl-review APPROVE）
  - SDD-D6-04 跨契约回归锁（**test-only，零运行时/src 代码**）：1 新 test 文件钉死 `CandidateB1SideNote.to_sidenote(nid).model_dump(mode="json")` → `ingest_side_note` 端到端 + 3 处脆弱点（created_at 须 json-mode ISO str / evidence_source==linked_source_refs / model_dump keys ⊇ render_side_note 必填集）；镜像 D6-01 `ing-1n` 跨契约锁模式；+8 tests（obsidian 77→85）；零 src/依赖/`__all__` 改动 → commit `f32ba7d`（codex spec-review round-1 REJECT-with-MAJOR 聚合多包跑 → rev2 round-2 APPROVE；impl-review MINOR-1 "sys.path 冗余" 经实测**驳回**——core 未 pip 安装，bootstrap 必需；spec 同步修 rev3）
  - 红线复核：R-3 §7.1 路径由 resolve_path 锁死（sources `{ns}/30_run_ledger/20_sources/{YYYY}/{MM}/` + side_notes `{ns}/20_episodic_memory/20_ai_observations/side_notes/{YYYY}/{MM}/`）、ModeEnforcer 拒 user_raw、不改用户原文；R-4 §7.2 create-only（`non_generated_overwrite_allowed=False` 硬锁）+ dry_run 默认。backend contract 仍 `1.4.0`（D6 不碰 backend / route / schema）；workspace 仍 7 包；contract_diff / private_leak_scan gate 全绿

### 当前

`PROJECT_DEFINITION_v3.2.md` 已取代旧蓝图作为 spec source of truth。δ demo day-0 (2026-05-12) 已闭环 5 SDD + 1 hotfix（D1-01 / D1-02 / D2-01 + hotfix-01 / D2-02 / D2-03）+ D1-02-hotfix-02 doc-only polish。Part 2.C 全部闭合：#1/#2/#3/#5 (D1-01/D1-02/D2-02/D2-03, day-0) + #6 candidate→confirmed gate 全局化 (D2-04 `fdf5f81`) + #4 SideNote opt-out 交互 (D2-05 `dbd5da4`) + #7 External Reference Pool 边界裁定 (radar scope = AI 元知识 only，领域 DB → 第二阶段)。δ day-2 D5-01..07 RPC 迁移完结，δ day-3 D6-01..04 vault 入库层完结。**v3.2 §10.4 范围裁决复核**：§4.2.2/4.2.3/4.2.4/4.2.5（反向投影 / 任务后反思 / DigestionItem 状态机 / 报告体系，即 §11.b 形态 2+3）经 §10.4 line 352/385 明确**延后至第一阶段之后**（"只做 b-1"）；phase-1 仅 b-1 主动旁注（=§10.2 demo 对象），已实现并 vault-wired/CI-locked。当前 (day-3/14) 焦点 = phase-1 MUP（§10.4 五判据）收口验证 + 14 天观察期，**不**建延后引擎（避免 Codex 审计 Condition #2 范围炸开）。phase-1 MUP gate record 见 `PHASE1_MUP_GATE.md`。14-day demo timer day-14=2026-05-26。

### 计划

- Part 2.C 全部闭合（#1-#7，详见 §当前）。δ day-3 D6-01..04 vault 入库层完结后，**v3.2 §10.4 范围复核裁定**：§4.2.2/4.2.3/4.2.4/4.2.5（反向投影 / 三层生成器 runtime / DigestionItem 状态机 / 报告体系）= §11.b 形态 2+3，blueprint §10.4 line 352/385 **明确延后至第一阶段之后**——不在 phase-1 建（违 §10.4 + R-5 = Codex 审计 Condition #2 范围炸开）。曾草拟的 D6-05..08 SDD 计划已**撤回**。
- δ 阶段焦点 = phase-1 MUP 收口（§10.4 五判据，gate record `PHASE1_MUP_GATE.md`）+ §10.2 14 天观察期（day-14=2026-05-26）：
  - 判据 1（§10.1 OMC demo）：PARTIALLY MET（test/contract 层证据，seed/fixture 驱动，无 committed live-run artifact）
  - 判据 2（§10.2 旁注 demo）：build-complete + infra-ready，acceptance **观察期 PENDING**（须 14 天内 ≥3 旁注 + ≥1 用户回应 + 全 metadata + 零 tone 违反，截 2026-05-26）
  - 判据 3/4/5（4 模块归属 / 4 端 health-check / 手工辩论→candidate）：MET（证据见 gate record）
...
- (§10.2) AI 旁注 metadata (依据 / 类型 / 置信度 / 用户认可) 已在 SideNote schema 定义；**vault 写入器已落地**（D6-03 `ingest_side_note` → R-3 旁注子树，D6-04 跨契约 CI 锁，2026-05-15 δ day-3）。剩余缺口在内容生成侧：§4.2.2 反向投影 / §4.2.3 三层质疑生成器（见 用户成长环）尚未实现，旁注内容仍非自动产出
