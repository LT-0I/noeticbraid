...
| 字段 | 值 |
|---|---|
| 当前阶段 | v3.2 δ demo day-0 (2026-05-12, 14-day timer day-14=2026-05-26)；Phase 1.2 SP integrations 完结；Part 2.C #1/#2/#3/#5 + D1-01 + D1-02 5 SDD 合并；§10.2 sidenote demo P1 verified；剩余 Part 2.C #4/#6/#7/#8 在 δ day-0 内推进 |
| 最后 noeticbraid commit (HEAD) | `f06b044` (`feat(omc-workspace): SDD-D2-03 capability 4-端 real health-check (δ-D5)`) |
| 最近 module integration commits | δ day-0 (2026-05-12) 共 6 commit：`f06b044` (D2-03 health-check) ← `b2ee841` (D2-02 OMC ingest workspace) ← `c16bc88` (D2-01-hotfix-01) ← `e1649e5` (D2-01 debate loop) ← `583307f` (δ day-0 docs) ← `a82ed88` (D1-02 b-1 detector)；γ 末 `059ef91` (D1-01 SideNote 加固 + contract 2.0.0) / `d480246` (γ status sync) |
| 当前 contract version | Backend API `1.3.0` (`packages/noeticbraid-backend/src/noeticbraid_backend/contracts.py::CONTRACT_VERSION`)；δ-C5 minor bump from 1.2.0 by SDD-D2-02 (additive: OMC workspace + capability registry surfaces)；1.2.0 byte-equal frozen snapshot 8 routes 保留 |
...
| Backend endpoints | 14 routes：8 frozen 1.2.0 (`FROZEN_ROUTE_SPECS`: health / auth / dashboard / workspace / approval / account / ledger runs / ledger aggregate) + 6 D2-02/D2-03 additive 1.3.0 (`OMC_WORKSPACE_ROUTE_SPECS`: omc-ingest project task GET/POST + project candidates GET + project adopted GET + project adopt POST + capabilities GET + capability health-check POST) |
| Schemas | 28 OpenAPI schema names (`ALL_SCHEMA_NAMES`)：17 frozen 1.2.0 + 11 D2-02/D2-03 additive (WorkspaceProject / CapabilityRegistryEntry / CapabilityHealthResult / CandidateLesson / OMCProjectTaskRequest / OMCProjectTaskResponse / OMCProjectCandidates / OMCProjectAdoptedHistory / CandidateAdoptionResponse / CapabilitiesResponse / CapabilityHealthCheckResponse)；另 SideNote v2 contract `CONTRACT_V2_VERSION=2.0.0` 由 SDD-D1-01 并存登记 |
| pytest core | PASS 485 (δ day-0 末记录；D2-03 δ-D5 跑) |
| pytest backend | PASS 101 (δ day-0 末记录；D2-03 δ-D5 跑) |
| pytest multimodel-alliance | PASS 46 (δ day-0 D2-01 δ-B5 跑) |
| console routes.test.tsx | PASS 12 (δ day-0 D2-03 δ-D5 跑) |
| CI 最近一次跑 | run `25381385524` PASS (last recorded gate; δ day-0 commits 未走远端 CI，全部本地 pytest 验证) |
| pending_actor (state.json) | `idle` in `noeticbraid-workflow/state.json`; δ day-0 6 commits 已落地 |
