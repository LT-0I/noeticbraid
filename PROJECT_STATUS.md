...
- `2026-05-15` (δ demo day-3): **D6 系列 SourceRecord → Obsidian vault 入库层落地**（SDD-D6-01 / D6-02，全程 dual-review）。补上 §10.2 旁注 demo 长期空缺的"vault 写入器"：D5 serializer 产出 SourceRecord dict → D6-01 `ingest_source_record`（单条 policy-checked 入库，commit `25cac32`）→ D6-02 `ingest_serialized_records`（批量幂等 fan-in + env→vault_root 解析收口 + 只读 `IngestSummary` 证据，commit `c79de72`）。obsidian `__all__` 19→22；obsidian pytest 34→64（D6-01 +18 / D6-02 +12）；5-包计量 1130，7-包合计 1180。零新策略（全委托）、零跨包改动、D6-02 零新依赖、workspace 仍 7 包；backend contract 仍 `1.4.0`（D6 不碰 backend）。红线全保：R-3 路径锁死 `30_run_ledger/20_sources/`、不改用户原文；R-4 create-only + dry_run 默认。HEAD `c79de72`（`25cac32` D6-01 ← `c79de72` D6-02）。所有 frozen contract byte-equal；contract_diff / private_leak_scan gate 全绿。详见 §2 "D6 系列 closures (2026-05-15)" + `1workflowR&D/specs/SDD-D6-0{1,2}-*.md`。
...
| pytest obsidian | PASS 64 (2026-05-15 δ day-3 全量重跑；D6-01 +18→52 / D6-02 +12→64；obsidian `__all__` 19→22) |
| pytest notebooklm-rpc | PASS 323 (D5-01..07；2026-05-14 δ day-2 全量重跑) |
| pytest workflow-scheduler / runtime | PASS 29 / 21 (沿用 δ day-2 值，D6 不碰这两包)；5-package 计量 1130 (rpc 323 + core 533 + backend 148 + multimodel 62 + obsidian 64)；7-package 合计 1180 |
...
| pending_actor (state.json) | `idle` in `noeticbraid-workflow/state.json`；δ day-3 D6-01/D6-02 + δ day-2 D5-01..07 + δ day-0..1 全部 SDD 已落地并 push origin main（tag `phase-1.2-SP-H-notebooklm-rpc-2.0.0` @ `9dcae5d` 未新增）；HEAD `c79de72` |
...
- **D6 系列 closures (2026-05-15, δ demo day-3)** — SourceRecord → Obsidian vault 入库层，全程 dual-review（codex spec-review 多轮 + code-reviewer impl-review），补 §10.2 旁注 demo "vault 写入器未实现" 缺口
  ...
  - 红线复核：R-3 §7.1 路径由 resolve_path 锁死 `{ns}/30_run_ledger/20_sources/{YYYY}/{MM}/`、ModeEnforcer 拒 user_raw、不改用户原文；R-4 §7.2 create-only（`non_generated_overwrite_allowed=False` 硬锁）+ dry_run 默认。backend contract 仍 `1.4.0`（D6 不碰 backend / route / schema）；workspace 仍 7 包；contract_diff / private_leak_scan gate 全绿
