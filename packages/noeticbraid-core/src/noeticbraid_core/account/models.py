...
_SECRET_ASSIGNMENT_RE = re.compile(
    r"(?i)\b(token|secret|password|passwd|api[_-]?key|authorization|cookie|session)\b\s*[:=]\s*[^\s,;]+"
)
# Bare "session" is intentionally not redacted here because _classify_signal()
# needs it to detect login_required. Assignment forms like session=<value> are
# still caught by _SECRET_ASSIGNMENT_RE above.
_SECRET_WORD_RE = re.compile(r"(?i)\b(secret|password|passwd|api[_-]?key|authorization|cookie|session[_-]?id|bearer)\b")
...
def _aware_utc(value: datetime) -> datetime:
    """Normalize a datetime to aware UTC, treating naive values as UTC."""
...
def sanitize_reason(value: str | None) -> str | None:
    ...
    This sanitizer deliberately keeps only a human-actionable category. Assignment-style
    sensitive values and common secret words are redacted before safe-character folding.
    """

    if value is None:
        return None
    value = _SECRET_ASSIGNMENT_RE.sub(lambda match: f"{match.group(1)}=[redacted]", value.strip())
    value = _SECRET_WORD_RE.sub("[redacted]", value)
    value = _SAFE_TEXT_RE.sub("_", value)
    value = " ".join(value.split())
    value = value.replace("[redacted]", "redacted")
    if not value:
        return None
    return value[:160]
