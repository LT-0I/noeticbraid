...
class AccountQuotaEnforcer:
    ...
    def record_usage(
        ...
        *,
        ...
        def _updater(state: dict[str, QuotaStateRecord]) -> None:
            ...
            if _would_limit_state(current, planned_increment):
                raise QuotaLimitExceeded(f"usage limit estimate would be exceeded for alias {alias}")
            usage_count = current.usage_count + planned_increment
            usage_window_started_at = current.usage_window_started_at or now
            remaining_estimate = _estimate_after_usage(
                usage_count,
                current.usage_limit_estimate,
                current.remaining_estimate,
            )
            state[alias] = current.model_copy(
                update={
        ...
        state_after = self.store.update_state(_updater)
        updated = state_after[alias]
        ...
        def _updater(state: dict[str, QuotaStateRecord]) -> None:
            ...
            state[alias] = current.model_copy(
                update={
                    ...
                    "remaining_estimate": remaining_estimate,
                    "cooldown_until": (
                        _as_aware_utc(cooldown_until) if cooldown_until is not None else current.cooldown_until
                    ),
    ...
    def _selection_key(
        ...
        return (
            ...
            -account.priority,  # negative for descending (higher priority first)
