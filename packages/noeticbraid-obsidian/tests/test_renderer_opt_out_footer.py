...
def test_side_note_footer_rendered() -> None:
    assert "\n---\n**opt-out**:" in _side_note_template()
...
def test_side_note_footer_contains_pause_command_literal() -> None:
    assert "noeticbraid b1-pause" in _side_note_template()


def test_side_note_footer_contains_rebut_command_literal() -> None:
    assert "noeticbraid b1-rebut --note-id=<note_id> --note-type=" in _side_note_template()
